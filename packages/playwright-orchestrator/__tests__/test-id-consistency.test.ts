...
describe("Test ID Consistency", () => {
  describe("File path format", () => {
    ...
    test("preserves subdirectory structure", () => {
      ...
      expect(testId).toBe("tests/e2e/features/auth/login.spec.ts::Login::should login");
    });

    test("same filename in different directories produces different IDs", () => {
      const id1 = buildTestId("e2e/auth/login.spec.ts", ["Login", "should login"]);
      const id2 = buildTestId("e2e/user/login.spec.ts", ["Login", "should login"]);
  ...
  describe("parseTestId roundtrip", () => {
    ...
    test("nested path roundtrips correctly", () => {
      const original = "tests/e2e/features/login.spec.ts::Auth::OAuth::should redirect";
      const parsed = parseTestId(original);
      const rebuilt = buildTestId(parsed.file, parsed.titlePath);
      expect(rebuilt).toBe(original);
    });
  ...
  describe("Playwright project support", () => {
    test("project name in titlePath creates unique ID", () => {
      ...
      const chromeId = buildTestId("e2e/login.spec.ts", ["chromium", "Login", "should login"]);
      const firefoxId = buildTestId("e2e/login.spec.ts", ["firefox", "Login", "should login"]);
...
describe("Simulated Component Consistency", () => {
  ...
  describe("Discovery consistency", () => {
    ...
    test("subdirectory test produces consistent ID", () => {
      ...
      expect(id1).toBe(id2);
      expect(id1).toBe("tests/e2e/features/auth/login.spec.ts::Auth::Login::should authenticate");
    });
  });
...
describe("Real-world scenarios", () => {
  test("monorepo with apps directory", () => {
    ...
    const testId = buildTestId("apps/web/e2e/login.spec.ts", ["Login", "should work"]);
    expect(testId).toBe("apps/web/e2e/login.spec.ts::Login::should work");
  });

  test("packages directory structure", () => {
    const testId = buildTestId("packages/ui/tests/button.spec.ts", ["Button", "renders correctly"]);
    expect(testId).toBe("packages/ui/tests/button.spec.ts::Button::renders correctly");
  });
