...
describe("toTestListFormat", () => {
  test("converts simple test entry (no describe block)", () => {
    expect(toTestListFormat({ file: "simple.spec.ts", titlePath: ["should work"] })).toBe(
      "simple.spec.ts › should work",
    );
  });
  ...
  test("handles empty testDirPrefix (testDir === rootDir)", () => {
    expect(
      toTestListFormat({ file: "login.spec.ts", titlePath: ["Login", "should login"] }, ""),
    ).toBe("login.spec.ts › Login › should login");
  });
  ...
  test("handles test name containing :: (internal separator)", () => {
    ...
    ).toBe("separator.spec.ts › Module::SubModule › login :: should authenticate user");
  });
});
...
describe("toTestListFile", () => {
  ...
  test("handles single test", () => {
    const result = toTestListFile([{ file: "simple.spec.ts", titlePath: ["should work"] }]);
  ...
  test("preserves :: in test names without corruption", () => {
    const result = toTestListFile([
      {
        file: "separator.spec.ts",
        titlePath: ["Separator Conflict Tests", "login :: should authenticate user"],
      },
      {
        file: "separator.spec.ts",
