...
function makeTimingData(tests: Record<string, { file: string; duration: number }>): TimingData {
...
describe("calculateFileAffinityPenalty", () => {
  test("returns default when timing data is null", () => {
    expect(calculateFileAffinityPenalty(null)).toBe(DEFAULT_FILE_AFFINITY_PENALTY);
  });

  test("returns default when timing data has no tests", () => {
    const td = makeTimingData({});
    expect(calculateFileAffinityPenalty(td)).toBe(DEFAULT_FILE_AFFINITY_PENALTY);
  });
...
describe("CKK with file affinity", () => {
  ...
  test("output expectedDurations reflect actual durations without penalties", () => {
    ...
    const totalActual = result.assignments.reduce((sum, a) => sum + a.expectedDuration, 0);
  ...
  test("penalty affects LPT shard selection preference", () => {
    ...
    const shardWithA = result.assignments.find((a) => a.tests.includes("a::big"));
...
describe("CKK dedup with file affinity", () => {
  test("explores shards with same load but different file sets", () => {
    ...
    const shardWithX1 = result.assignments.find((a) => a.tests.includes("x::t1"));
...
describe("LPT tiebreaking", () => {
  test("prefers shard with same file when loads are equal", () => {
    ...
    const shardWithA1 = result.assignments.find((a) => a.tests.includes("a::t1"));
    const shardWithB1 = result.assignments.find((a) => a.tests.includes("b::t1"));
