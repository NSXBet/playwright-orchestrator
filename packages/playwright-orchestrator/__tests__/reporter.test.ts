...
describe("Test ID Format", () => {
  describe("buildTestId", () => {
    test("builds ID from file and single describe", () => {
      const testId = buildTestId("e2e/login.spec.ts", ["Login", "should login"]);
    ...
    test("builds ID with nested describes", () => {
      ...
      expect(testId).toBe("e2e/auth.spec.ts::Auth::OAuth::Google::should redirect");
    });
    ...
    test("preserves forward slashes in file path", () => {
      const testId = buildTestId("tests/e2e/features/login.spec.ts", ["Login", "should login"]);
      expect(testId).toBe("tests/e2e/features/login.spec.ts::Login::should login");
    });
  });
  ...
  describe("parseTestId", () => {
    ...
    test("parses nested test ID", () => {
      const result = parseTestId("e2e/auth.spec.ts::Auth::OAuth::Google::should redirect");
      expect(result.file).toBe("e2e/auth.spec.ts");
      expect(result.titlePath).toEqual(["Auth", "OAuth", "Google", "should redirect"]);
    });
  });
});
...
describe("PROBLEM 1: Substring Collision Prevention", () => {
  test("should NOT match prefix substring", () => {
    ...
    expect(allowedTests.has("login.spec.ts::Login::should login with SSO")).toBe(false);
    expect(allowedTests.has("login.spec.ts::Login::should login successfully")).toBe(false);
    expect(allowedTests.has("login.spec.ts::Login::should login as admin")).toBe(false);
  });
  ...
  test("should NOT match suffix substring", () => {
    ...
    expect(allowedTests.has("api.spec.ts::API::should fetch users")).toBe(false);
  ...
  test("should NOT match middle substring", () => {
    ...
    expect(allowedTests.has("test.spec.ts::Suite::gracefully handles error case")).toBe(false);
    expect(allowedTests.has("test.spec.ts::Suite::properly handles error response")).toBe(false);
  });
  ...
  test("should differentiate between similar test names", () => {
    ...
    expect(allowedTests.has("api.spec.ts::API::should fetch user data")).toBe(false);
    expect(allowedTests.has("api.spec.ts::API::should fetch usernames")).toBe(false);
  });
...
describe("PROBLEM 2: Bash Syntax Error Characters", () => {
  describe("Parentheses () - The original bet-app error", () => {
    test("handles parentheses in test name", () => {
      ...
      expect(
        allowedTests.has("betslip.v2.spec.ts::BetSlip v2::should show message for (ServerMessage)"),
      ).toBe(true);
    });

    test("handles multiple parentheses", () => {
      const allowedTests = new Set(["test.spec.ts::Test::function(arg1, arg2) returns (result)"]);

      expect(allowedTests.has("test.spec.ts::Test::function(arg1, arg2) returns (result)")).toBe(
        true,
      );
    });

    test("handles nested parentheses", () => {
      const allowedTests = new Set(["test.spec.ts::Test::should handle ((nested)) parens"]);

      expect(allowedTests.has("test.spec.ts::Test::should handle ((nested)) parens")).toBe(true);
    });

    test("handles empty parentheses", () => {
      const allowedTests = new Set(["test.spec.ts::Test::function() call"]);

      expect(allowedTests.has("test.spec.ts::Test::function() call")).toBe(true);
    });
  });
  ...
  describe("Square Brackets [] - Array/index notation", () => {
    test("handles square brackets", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::should access array[0]")).toBe(true);
      expect(allowedTests.has("test.spec.ts::Test::handles [object Object]")).toBe(true);
      expect(allowedTests.has("test.spec.ts::Test::parses [1, 2, 3]")).toBe(true);
    });
  });
  ...
  describe("Curly Braces {} - Object notation", () => {
    test("handles curly braces", () => {
      ...
      const templateVar = "test.spec.ts::Test::template $" + "{variable} substitution";
      const allowedTests = new Set([
      ...
      expect(allowedTests.has("test.spec.ts::Test::should parse {key: value}")).toBe(true);
      expect(allowedTests.has("test.spec.ts::Test::renders {} empty object")).toBe(true);
  ...
  describe("Semicolons ; - Command separator", () => {
    test("handles semicolons", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::a; b; c sequence")).toBe(true);
    });
  });
  ...
  describe("Ampersands & - Background/AND operator", () => {
    ...
    test("handles double ampersand (AND)", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::condition && result")).toBe(true);
    });
  });
  ...
  describe("Angle Brackets <> - Redirects/generics", () => {
    test("handles less than", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::a < b comparison")).toBe(true);
    });
    ...
    test("handles HTML-like tags", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::renders <Component />")).toBe(true);
      expect(allowedTests.has("test.spec.ts::Test::parses <div>content</div>")).toBe(true);
      expect(allowedTests.has("test.spec.ts::Test::handles Array<string>")).toBe(true);
    });
  });
  ...
  describe("Wildcards * ? - Glob patterns", () => {
    test("handles asterisks", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::matches *.ts files")).toBe(true);
      expect(allowedTests.has("test.spec.ts::Test::a * b multiplication")).toBe(true);
    });
    ...
    test("handles question marks", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::what? why? how?")).toBe(true);
    });
  });
  ...
  describe("Exclamation marks ! - Negation/history", () => {
    test("handles exclamation marks", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::should not! fail!")).toBe(true);
      expect(allowedTests.has("test.spec.ts::Test::!important assertion")).toBe(true);
    });
  });
});
...
describe("PROBLEM 3: Shell Escaping Characters", () => {
  describe("Dollar Sign $ - Variable expansion", () => {
    test("handles dollar sign (currency)", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::should format $100.00")).toBe(true);
    });
    ...
    test("handles dollar sign (variable-like)", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::handles $variable")).toBe(true);
  ...
  describe("Backticks ` - Command substitution", () => {
    test("handles backticks (code blocks)", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::should render `code` blocks")).toBe(true);
      expect(allowedTests.has("test.spec.ts::Test::parses `inline code`")).toBe(true);
    });
  });
  ...
  describe("Single Quotes ' - String delimiter", () => {
    test("handles single quotes", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::should show 'warning' message")).toBe(true);
  ...
  describe('Double Quotes " - String delimiter', () => {
    test("handles double quotes", () => {
      ...
      expect(allowedTests.has('test.spec.ts::Test::should handle "quoted" text')).toBe(true);
    });
  });
  ...
  describe("Backslashes \\ - Escape character", () => {
    test("handles backslashes", () => {
      ...
      expect(allowedTests.has("test.spec.ts::Test::regex \\d+ pattern")).toBe(true);
    });
  });
});
...
describe("PROBLEM 4: Pipe Character Issues", () => {
  test("handles single pipe (OR in regex, bash pipe)", () => {
    ...
    expect(allowedTests.has("test.spec.ts::Test::should parse A | B | C")).toBe(true);
  });
  ...
  test("handles double pipe (logical OR)", () => {
    ...
    expect(allowedTests.has("test.spec.ts::Test::condition || fallback")).toBe(true);
  });
  ...
  test("handles pipe in table/separator context", () => {
    ...
    expect(allowedTests.has("test.spec.ts::Test::renders | header | row |")).toBe(true);
  });
});
...
describe("PROBLEM 5: Parameterized Test Support (test.each)", () => {
  ...
  test("handles test.each with object parameters", () => {
    ...
    expect(allowedTests.has("test.spec.ts::Suite::object with a=3")).toBe(false);
  });
  ...
  test("handles test.each with complex titles", () => {
    ...
    expect(allowedTests.has("test.spec.ts::Suite::adds 1 to 1 to get 2")).toBe(true);
    expect(allowedTests.has("test.spec.ts::Suite::adds 1 to 2 to get 3")).toBe(true);
  });

  test("handles describe.each with nested tests", () => {
    // Simulate describe.each([1, 2])('suite %i', () => { test('works', ...) })
    const allowedTests = new Set(["test.spec.ts::suite 1::works", "test.spec.ts::suite 2::works"]);
...
describe("PROBLEM 6: Real-World Production Test Names", () => {
  ...
  test("HTTP status codes in parentheses", () => {
    ...
    expect(allowedTests.has("api.spec.ts::API::should handle error (400)")).toBe(true);
    expect(allowedTests.has("api.spec.ts::API::should handle error (500)")).toBe(true);
    // Should not match other codes not in the set
    expect(allowedTests.has("api.spec.ts::API::should handle error (502)")).toBe(false);
  });
  ...
  test("currency formatting", () => {
    ...
    expect(allowedTests.has("payment.spec.ts::Payment::formats $100.00 correctly")).toBe(true);
    expect(allowedTests.has("payment.spec.ts::Payment::handles R$50,00 BRL format")).toBe(true);
  });
  ...
  test("feature flags with brackets", () => {
    ...
    expect(allowedTests.has("features.spec.ts::Features::should enable [FEATURE_A]")).toBe(true);
  });
  ...
  test("user-facing messages with quotes", () => {
    ...
    expect(allowedTests.has('ui.spec.ts::UI::displays "Success!" message')).toBe(true);
    expect(allowedTests.has("ui.spec.ts::UI::shows 'Warning' alert")).toBe(true);
  });
  ...
  test("API endpoints with special chars", () => {
    ...
    expect(allowedTests.has("routes.spec.ts::Routes::GET /api/users?page=1&limit=10")).toBe(true);
    expect(allowedTests.has("routes.spec.ts::Routes::POST /api/users/{id}/profile")).toBe(true);
  });
});
...
describe("PROBLEM 7: Unicode and International Characters", () => {
  test("handles emojis", () => {
    ...
    expect(allowedTests.has("test.spec.ts::Test::shows 🎉 celebration")).toBe(true);
    expect(allowedTests.has("test.spec.ts::Test::renders ❌ error icon")).toBe(true);
  });
  ...
  test("handles non-ASCII characters", () => {
    ...
    expect(allowedTests.has("test.spec.ts::Test::displays café menu")).toBe(true);
    expect(allowedTests.has("test.spec.ts::Test::renders 日本語 text")).toBe(true);
  });
  ...
  test("handles mathematical symbols", () => {
    ...
    expect(allowedTests.has("test.spec.ts::Test::calculates π value")).toBe(true);
    expect(allowedTests.has("test.spec.ts::Test::shows ≤ and ≥ operators")).toBe(true);
  });
});
...
describe("JSON Shard File Format", () => {
  test("parses valid shard file format", () => {
    ...
    expect(allowedTests.has("e2e/login.spec.ts::Login::should login")).toBe(true);
    expect(allowedTests.has("e2e/home.spec.ts::Home::should render")).toBe(true);
  });
  ...
  test("handles large number of test IDs efficiently", () => {
    ...
    const testIds = Array.from({ length: 1000 }, (_, i) => `test.spec.ts::Suite::test ${i}`);
...
describe("Edge Cases and Boundary Conditions", () => {
  ...
  test("handles test title with :: separator (edge case)", () => {
    ...
    const allowedTests = new Set(["test.spec.ts::Suite::title :: with :: colons"]);

    expect(allowedTests.has("test.spec.ts::Suite::title :: with :: colons")).toBe(true);
