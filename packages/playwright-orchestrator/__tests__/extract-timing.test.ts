...
import type { PlaywrightReport, ShardTimingArtifact } from "../src/core/types.js";
...
describe("Extract Timing Command", () => {
  function runExtractTiming(report: PlaywrightReport, project = "default"): ShardTimingArtifact {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "extract-timing-test-"));
  ...
  describe("Root suite title excluded from titlePath", () => {
    test("filename does not appear twice in test ID", () => {
                ...
                specs: [
                  {
                    ...
                    tests: [{ results: [{ duration: 1500, status: "passed" }] }],
                  },
                ],
              },
            ],
      ...
      const expectedTestId = "account.spec.ts::Account Page::should render initial tab by default";
    ...
    test("nested describe blocks work correctly", () => {
      ...
      const expectedTestId = "login.spec.ts::Login::OAuth::should redirect to provider";
      const testIds = Object.keys(result.tests);
      expect(testIds[0]).toBe(expectedTestId);
    });
  });
  ...
  describe("Path resolution", () => {
            ...
            suites: [
              {
                ...
                specs: [
                  {
                    title: "should render",
                    tests: [{ results: [{ duration: 1000, status: "passed" }] }],
                  },
                ],
              },
            ],
    ...
    test("deep directory structure resolves correctly", () => {
                ...
                specs: [
                  {
                    ...
                    tests: [{ results: [{ duration: 3000, status: "passed" }] }],
                  },
                ],
              },
            ],
      ...
      const result = runExtractTiming(report);
      const expectedTestId = "features/checkout.spec.ts::Checkout::should process payment";
      const testIds = Object.keys(result.tests);
      expect(testIds[0]).toBe(expectedTestId);
    });
  });
  ...
  describe("Consistency with test-discovery", () => {
    test("produces same IDs as test-discovery", () => {
                    ...
                    specs: [
                      {
                        ...
                        tests: [{ results: [{ duration: 500, status: "passed" }] }],
                      },
                    ],
                  },
                ],
      ...
      const result = runExtractTiming(report);
      const expectedTestId = "e2e/api.spec.ts::API::REST::GET /users returns 200";
      const testIds = Object.keys(result.tests);
      expect(testIds[0]).toBe(expectedTestId);
    });
  });
